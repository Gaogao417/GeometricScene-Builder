...
try:
    ...
    print(json.dumps({"status": "error", "message": f"Missing plotting libraries: {e}"}))
    sys.exit(1)
...
def generate_analysis(results: List[Dict], ratings: Dict, run_dir: Path) -> str:
    ...
    df = df.merge(pd.DataFrame(list(ratings.values())), on='key', how='left', suffixes=('', '_rating'))
...
"""
    ...
    recipe_stats = df.groupby('recipe_name').agg({
        'success': ['count', 'sum'],
        'solve_time_s': ['mean', 'median']
    }).round(2)

    recipe_stats.columns = ['total', 'successful', 'mean_time', 'median_time']
    recipe_stats['success_rate'] = (recipe_stats['successful'] / recipe_stats['total'] * 100).round(1)
...
def generate_figures(results: List[Dict], ratings: Dict, run_dir: Path):
    ...
    fig2 = px.box(df[df['success']], x='recipe_name', y='solve_time_s',
    ...
    if 'rating' in df.columns and df['rating'].notna().any():
        ...
        fig3 = px.scatter(rated, x='solve_time_s', y='rating',
                          color='recipe_name',
