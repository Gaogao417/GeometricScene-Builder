...
description: Use this skill when generating a single teaching-oriented geometry diagram through the lightweight agent/tool loop: create a candidate scene/spec, render it, evaluate image usability, revise from defects, and stop after the retry budget.
---
...
DiagramRequest
  ...
  -> rendered image
  -> VLM usability evaluation
  -> defect-driven revision, at most 3 revisions
  -> final diagram-spec/image/logs
```
...
3. If `status=ok`, pass `final_diagram_spec.json` and the final image to the teaching layer.
...
3. Call `render_diagram_candidate(request_path, scene_payload_path, out_dir?, round_index=0)`.
4. Call `evaluate_diagram_image(request_path, render_result_path, out_dir?, round_index=0)`.
...
- Prefer an analytic `diagram-spec` and deterministic coordinate renderer.
